...
// 0.05 u
	...
	LPC_SC -> PCLKSEL0 |= (1<<2); 		// "01" -> CLKTMR = CCLK

	LPC_TIM0 -> MR0 = 828;		// Valor de carga p 10 [mseg]
...
void aConvertir (int32_t valor)
{
	valor = (int) (valor/20) + 500;
	//LPC_DAC -> DACR = ((valor & 0x3FF) << 6); // Escribo valor en registro para convertir
	LPC_DAC -> DACR = (valor << 6); // Escribo valor en registro para convertir
